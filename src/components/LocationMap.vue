...
const initMap = () => {
  ...
  if (props.useGeolocation) {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      error => {
        console.error(t('map.locationError'), error);
        // 如果獲取位置失敗，使用預設位置
  ...
  props.markers.forEach(markerData => {
    ...
    marker.bindPopup(
      ...
      <div class="p-2">
        ...
        ${
          markerData.rating
            ? `<p class="text-sm">${t('map.rating')}: ${markerData.rating}</p>`
            : ''
        }
        <div class="mt-2">
          <select id="daySelect_${
            markerData.id
            ...
            ${props.tripDays
              .map(
                day =>
                  `<option value="${day.id}" ${
                    day.id === props.selectedDay ? 'selected' : ''
                  }>${t('planner.day', { n: day.id })}</option>`
              )
              .join('')}
          </select>
          <button onclick="window.addToTrip('${
            markerData.id
          }')" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">${t(
        'search.addToTrip'
      )}</button>
        </div>
      </div>
      ...
      <div class="leaflet-tooltip-content">
        ...
        ${
          markerData.description
            ? `<div class="text-sm">${markerData.description}</div>`
            : ''
        }
      </div>
    `,
