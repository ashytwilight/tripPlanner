...
const regionCenters = {
  ...
  'UK': { lat: 55.3781, lon: -3.4360, name: '英國' },
  'KR': { lat: 35.9078, lon: 127.7669, name: '韓國' },
  'SG': { lat: 1.3521, lon: 103.8198, name: '新加坡' },
  'TH': { lat: 15.8700, lon: 100.9925, name: '泰國' }
}

export const searchAttractions = async (query = '', region = 'TW', options = {}) => {
  try {
      ...
      return response.data.elements
        ...
        .map(place => ({
          ...
          description: place.tags.description || [
            ...
            place.tags['addr:housenumber'] ? place.tags['addr:housenumber'] + '號' : null,
            place.tags['addr:floor']
          ]
            .filter(Boolean) 
            .join('') ||`位於${regionCenters[region].name}的${place.tags.name}`,
          rating: 0,
          location: {
    ...
    if (error.response) {
      switch (error.response.status) {
        case 400:
          throw new Error('搜尋參數無效，請修改搜尋條件')
        case 401:
          throw new Error('API授權失敗，請聯繫管理員')
        case 429:
          throw new Error('請求次數過多，請稍後再試')
        case 500:
          throw new Error('服務器錯誤，請稍後再試')
        default:
          throw new Error(`服務器返回錯誤 (${error.response.status})`)
      }
    } else if (error.request) {
      throw new Error('網絡連接失敗，請檢查網絡設置')
    } else {
      throw new Error('搜尋景點時發生錯誤，請稍後再試')
    }
  }
}