  ...
  <div class="py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl mx-auto">
      <div class="flex justify-between items-center mb-8 px-6">
        <h1
          class="text-4xl font-bold text-gray-900 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600"
        >
          {{ t('planner.title') }}
        </h1>
        <div class="space-x-4">
      ...
      <div v-if="loading" class="text-center py-12">
        <div
          class="animate-spin rounded-full h-16 w-16 border-4 border-blue-500 border-t-transparent mx-auto"
        ></div>
      </div>
      ...
      <div v-else class="space-y-8">
        ...
        >
          <h2 class="text-xl font-bold text-gray-900 mb-6 flex items-center">
            <span
              class="bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-full w-7 h-7 flex items-center justify-center mr-3 text-base"
              >{{ day.id }}</span
            >
            {{ t('planner.day', { n: day.id }) }}
            <button
              @click="removeDay(dayIndex)"
              class="ml-auto flex items-center px-3 py-1 text-red-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors duration-200 text-lg"
            >
              <svg
                class="w-5 h-5 mr-1"
                fill="none"
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <div class="flex items-center space-x-4 mt-4">
                <select
                  ...
                  @change="
                    moveAttraction(dayIndex, $event.target.value, attraction)
                  "
                >
                  <option value="">{{ t('planner.switch') }}</option>
                  <option
            ...
            <div class="mt-6 bg-white rounded-xl p-6 shadow-sm">
              ...
              <textarea
                ...
                :placeholder="t('planner.textareaPlaceholder', { n: day.id })"
              ></textarea>
            </div>
          </div>
        </div>
...
const moveAttraction = (fromDay, toDay, attraction) => {
  ...
  if (
    fromDayIndex >= 0 &&
    fromDayIndex < days.value.length &&
    toDayIndex >= 0 &&
    toDayIndex < days.value.length
  ) {
    const fromDayAttractions = days.value[fromDayIndex].attractions;
    const attractionIndex = fromDayAttractions.findIndex(
