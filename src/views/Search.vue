...
const addToTrip = (attraction) => {
  // 檢查選擇的天數是否存在
  const dayIndex = tripDays.value.findIndex(day => day.id === selectedDay.value)
  if (dayIndex === -1) {
    alert('請選擇有效的行程天數')
    return
  }
...
<!-- {{mapBounds  }} -->
      <div class="flex flex-col md:flex-row gap-4 mb-8">
        <div class="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <div class="flex gap-2 w-full sm:w-auto">
            <button
              ...
              class="px-4 py-2 rounded flex-1 sm:flex-none"
              :class="!isMapMode ? 'bg-blue-500 text-white' : 'bg-gray-200'"
            >
              列表模式
            </button>
            <button
        ...
        <div v-if="isMapMode" class="flex gap-2 overflow-x-auto w-full md:w-auto">
          <button
            v-for="cat in ['attractions', 'food', 'cafe', 'hotel', 'park', 'shopping']"
            :key="cat"
            @click="() => { selectedCategory = cat; if (mapBounds) handleSearch(); }"
            class="px-4 py-2 whitespace-nowrap rounded flex-shrink-0"
            :class="selectedCategory === cat ? 'bg-green-500 text-white' : 'bg-gray-200'"
          >
            {{
              {
                attractions: '景點',
      ...
      <div v-if="isMapMode" class="relative mb-6 flex flex-col md:flex-row">
        <!-- 搜尋結果側邊欄 -->
        <!-- <div class="w-full md:w-1/3 bg-white shadow-lg overflow-y-auto h-[300px] md:h-[600px] md:mr-4">
          ...
          <div v-else class="p-4 space-y-4">
            <div v-for="attraction in attractions" :key="attraction.id"
              ...
              @click="() => {
                $refs.locationMap.panTo(attraction.location.lat, attraction.location.lng);
                $refs.locationMap.openMarkerPopup(attraction.id);
              }">
              <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ attraction.name }}</h3>
              <p class="text-sm text-gray-600 mb-2">{{ attraction.description }}</p>
              <div class="flex justify-between items-center">
                <span class="text-xs text-gray-500">{{ attraction.city }}</span>
                <div class="flex items-center gap-2">
                  <select
                    v-model="selectedDay"
                    class="text-sm px-2 py-1 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                  ...
                  <button
                    @click.stop="addToTrip(attraction)"
        ...
        <div class="flex-1 relative">
          <div v-if='mapLoading' class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[500]">
            <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500"></div>
          </div>
          <LocationMap
            ref="locationMap"
            ...
            @update:bounds="(bounds) => { mapBounds = bounds; debouncedSearch() }"
      ...
      <!-- <div v-else-if="attractions.length === 0 && !loading" class="text-center py-8 text-gray-500">
        {{ t('search.noResults') }}
      </div> -->
